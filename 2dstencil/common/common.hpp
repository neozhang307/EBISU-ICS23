// from https://github.com/pssrawat/IEEE2017
...
template <>
double4 get_random<double4>()
{
  double4 random;
  random.x = get_random<double>();
  random.y = get_random<double>();
  random.z = get_random<double>();
  random.w = get_random<double>();
  return random;
}
template <>
...
{
  T(*a)
  [width_x] = (T(*)[width_x]) new T[width_y * width_x];
...
{
  T(*a)
  [height][width_y][width_x] = (T(*)[height][width_y][width_x]) new T[depth * height * width_y * width_x];
...
template <typename T>
static double checkError2D(int width_x, const T *l_output, const T *l_reference, int y_lb, int y_ub,
                           int x_lb, int x_ub)
{
...
template <typename T>
static double checkError3D(int width_y, int width_x, const T *l_output, const T *l_reference, int z_lb,
...
template <typename T>
static double checkError4D(int height, int width_y, int width_x, const T *l_output, const T *l_reference, int w_lb, int w_ub, int z_lb,
                           int z_ub, int y_lb, int y_ub, int x_lb, int x_ub)
{
  ...
  printf("[Test] Max Error : %e @ (%d,%d,%d,%d)\n", max_error, max_d, max_i, max_j, max_k);
  error = sqrt(error / ((w_ub - w_lb) * (z_ub - z_lb) * (y_ub - y_lb) * (x_ub - x_lb)));
  return error;
}

template <>
double checkError3D<double4>(int width_y, int width_x, const double4 *l_output, const double4 *l_reference,
                             int z_lb, int z_ub, int y_lb, int y_ub, int x_lb, int x_ub)
{
  const double4(*output)[width_y][width_x] =
      (const double4(*)[width_y][width_x])(l_output);
  const double4(*reference)[width_y][width_x] =
      (const double4(*)[width_y][width_x])(l_reference);
  double error = 0.0;
  double max_error = 0.0;
  ...
  printf("[Test] Max Error : %e @ (%d,%d,%d,%c)\n", max_error, max_i, max_j,
         max_k,
         (max_field == 0 ? 'x' : (max_field == 1 ? 'y' : (max_field == 2 ? 'z' : 'w'))));
  error = sqrt(error / (4 * (z_ub - z_lb) * (y_ub - y_lb) * (x_ub - x_lb)));
  return error;
}
template <>
double checkError3D<float4>(int width_y, int width_x, const float4 *l_output, const float4 *l_reference,
                            int z_lb, int z_ub, int y_lb, int y_ub, int x_lb, int x_ub)
{
...
template <typename T>
static void print2DArray(int width_x, const T *l_array, int y_lb, int y_ub, int x_lb, int x_ub)
{
...
template <typename T>
static void print3DArray(int width_y, int width_x, const T *l_array, int z_lb, int z_ub, int y_lb,
...
template <>
void print3DArray<float4>(int width_y, int width_x, const float4 *l_array, int z_lb, int z_ub, int y_lb,
                          int y_ub, int x_lb, int x_ub)
{
  ...
  for (int i = z_lb; i < z_ub; i++)
  {
    ...
    for (int j = y_lb; j < y_ub; j++)
    {
      printf("\t");
      for (int k = x_lb; k < x_ub; k++)
        printf(" [%e, %e, %e, %e]", array[i][j][k].x, array[i][j][k].y,
