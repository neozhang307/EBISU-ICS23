...
static void jacobi_step(const REAL *l_input, int width_y, int width_x, REAL *l_output, int step)
{
  ...
  for (int i = 3; i < width_y - 3; i++)
    for (int j = 3; j < width_x - 3; j++)
      output[i][j] = (7 * input[i - 3][j] + 7 * input[i - 2][j] + 5 * input[i - 1][j] + 9 * input[i][j - 3] + 9 * input[i][j - 2] + 12 * input[i][j - 1] + 15 * input[i][j] + 12 * input[i][j + 1] + 9 * input[i][j + 2] + 9 * input[i][j + 3] + 5 * input[i + 1][j] + 7 * input[i + 2][j]+ 7 * input[i + 3][j]) / 118;
}

template <class REAL>
void jacobi_gold(REAL *l_input, int width_y, int width_x, REAL *l_output)
{
