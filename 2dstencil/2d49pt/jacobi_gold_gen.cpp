...
(const REAL* l_input, int width_y, int width_x, REAL* l_output, int step)
{
  ...
  for( int l_y = 0 ; l_y < width_y-0 ; l_y++ )
  {
    for( int l_x = 0 ; l_x < width_x-0 ; l_x++ )
    {
      ...
      int w2 = (l_x-3>0?l_x-3:0);//(l_x == 0)        ? 0 : 0 - 1;
      int e0 = (l_x+1<width_x-1?l_x+1:width_x-1);//(l_x == width_x-1)     ? 0 : 0 + 1;
      int e1 = (l_x+2<width_x-1?l_x+2:width_x-1);//(l_x == width_x-1)     ? 0 : 0 + 1;
      int e2 = (l_x+3<width_x-1?l_x+3:width_x-1);//(l_x == width_x-1)     ? 0 : 0 + 1;
      int s0 = (l_y-1>0?l_y-1:0);//(l_y == 0)        ? 0 : 0 - 1;
      int s1 = (l_y-2>0?l_y-2:0);//(l_y == 0)        ? 0 : 0 - 1;
      int s2 = (l_y-3>0?l_y-3:0);//(l_y == 0)        ? 0 : 0 - 1;
      int n0 = (l_y+1<width_y-1?l_y+1:width_y-1);;//(l_y == width_y-1)     ? 0 : 0 + 1;    
      int n1 = (l_y+2<width_y-1?l_y+2:width_y-1);;//(l_y == width_y-1)     ? 0 : 0 + 1;    
      int n2 = (l_y+3<width_y-1?l_y+3:width_y-1);;//(l_y == width_y-1)     ? 0 : 0 + 1;    

      output[l_y][l_x] = 
      ( 
        1*  input[s2][w2] +1*  input[s2][w1] + 2*  input[s2][w0] + 3*  input[s2][l_x] + 4*input[s2] [e0] + 5*input[s2] [e1] + 5*input[s2] [e2] + 
        1*  input[s1][w2] +1*  input[s1][w1] + 2*  input[s1][w0] + 3*  input[s1][l_x] + 4*input[s1] [e0] + 5*input[s1] [e1] + 5*input[s1] [e2] + 
        1*  input[s0][w2] +7*  input[s0][w1] + 7*  input[s0][w0] + 5*  input[s0][l_x] + 7*input[s0] [e0] + 6*input[s0] [e1] + 5*input[s0] [e2] + 
        1*  input[l_y][w2] +8* input[l_y][w1] + 12* input[l_y][w0] + 15.0* input[l_y][l_x] + 12* input[l_y][e0] + 12* input[l_y][e1] +5*input[l_y] [e2] + 
        1*  input[n0][w2] +9*  input[n0][w1] + 9*  input[n0][w0] + 5*  input[n0][l_x] + 7*  input[n0][e0] + 15*  input[n0] [e1] +5*input[n0] [e2] + 
        1*  input[n1][w2] +9*  input[n1][w1] + 9*  input[n1][w0] + 5*  input[n1][l_x] + 7*  input[n1][e0] + 15*  input[n1] [e1] +5*input[n1] [e2] + 
        1*  input[n2][w2] +10*  input[n2][w1] + 11*  input[n2][w0] + 12*  input[n2][l_x] + 13*  input[n2][e0] + 14*  input[n2] [e1]+5*input[n2] [e2] 
        ) / 118;  
    }  
  } 
}
