...
(const REAL* l_input, int width_y, int width_x, REAL* l_output, int step)
{
  ...
  REAL (*output)[width_x] = (REAL (*)[width_x])l_output;
  for( int i = 3 ; i < width_y-3; i++ )
    for( int j = 3 ; j < width_x-3 ; j++ )
      output[i][j] = (
        1*input[i-3][j-3] + 1*input[i-3][j-2] + 2*input[i-3][j-1] + 3*input[i-3][j] + 4*input[i-3][j+1] + 5*input[i-3][j+2] +5*input[i-3][j+3] +
        1*input[i-2][j-3] + 1*input[i-2][j-2] + 2*input[i-2][j-1] + 3*input[i-2][j] + 4*input[i-2][j+1] + 5*input[i-2][j+2] +5*input[i-2][j+3] +
        1*input[i-1][j-3] + 7*input[i-1][j-2] + 7*input[i-1][j-1] + 5*input[i-1][j] + 7*input[i-1][j+1] + 6*input[i-1][j+2] +5*input[i-1][j+3] +
        1*input[i][j-3] + 8*input[i][j-2] + 12*input[i][j-1] + 15*input[i][j] + 12*input[i][j+1] +  12*input[i][j+2] + 5*input[i][j+3] +
        1*input[i+1][j-3] + 9*input[i+1][j-2] + 9*input[i+1][j-1] + 5*input[i+1][j] + 7*input[i+1][j+1] + 15*input[i+1][j+2] +5*input[i+1][j+3] +
        1*input[i+2][j-3] + 9*input[i+2][j-2] + 9*input[i+2][j-1] + 5*input[i+2][j] + 7*input[i+2][j+1] + 15*input[i+2][j+2] +5*input[i+2][j+3] +
        1*input[i+3][j-3] + 10*input[i+3][j-2] + 11*input[i+3][j-1] + 12*input[i+3][j] + 13*input[i+3][j+1] + 14*input[i+3][j+2]+5*input[i+3][j+3] 
        ) 
    / 118;
